...
const UI_TRANSLATIONS = {
  jp: {
    ...
    settingsBenefitLocalTitle: '🚨 未設定 (ローカルエンジン)',
    settingsBenefitLocalDesc: '単純なキーワード抽出によるテンプレート差し替えのみ。生成パターンが固定（各モード約6パターン）されているため、何度も使うと同じようなメッセージになりがちです。',
    settingsBenefitAiTitle: '🔥 設定あり (Gemini AIエンジン)',
    settingsBenefitAiDesc: 'AIが入力されたコミットの内容（例：ログイン不具合の修正など）を完全に読解。その不具合が起きた悲劇的背景や開発者の葛藤を、毎回その場でオリジナル執筆します。生成される内容は無限パターンです。',
  ...
  en: {
    ...
    settingsBenefitLocalTitle: '🚨 Unconfigured (Local Engine)',
    settingsBenefitLocalDesc: 'Simple keyword search and template injection. Patterns are fixed (about 6 per mode) and might feel repetitive over time.',
    settingsBenefitAiTitle: '🔥 Configured (Gemini AI Engine)',
    settingsBenefitAiDesc: 'AI reads and fully understands the context of your changes. It dynamically invents contextually relevant jokes, developer anxiety, and narratives. Generated patterns are infinite.',
